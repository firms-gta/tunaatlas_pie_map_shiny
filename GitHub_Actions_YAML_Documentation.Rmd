---
title: "GitHub Actions YAML Documentation"
output:
...
```yaml
  - name: Calculate DOI.csv hash
    id: doi_csv
    run: echo "doi_csv_hash=$(sha256sum data/DOI.csv | cut -d' ' -f1)" >> $GITHUB_ENV
```

### Explanation:
- Similar to the `renv.lock` step but calculates the hash for `data/DOI.csv`.
