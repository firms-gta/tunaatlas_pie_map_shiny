...
# Step 5: Build the shiny app in local 
...
```bash
docker buildx build \
  --cache-from type=registry,ref=ghcr.io/<your_repo>/<cache_name>:latest \
  --cache-to type=registry,ref=ghcr.io/<your_repo>/<cache_name>:latest,mode=max \
  -t <your_image_name>:latest .
```

- Replace `<your_repo>` with your GitHub repository.
- Replace `<cache_name>` with your desired cache name.
- Replace `<your_image_name>` with your image name.
